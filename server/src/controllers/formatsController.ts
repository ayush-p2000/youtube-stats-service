...
const extractQuality = (resolution: string | undefined, note: string | undefined): string | undefined => {
    if (resolution) return resolution;
    if (note) {
        const qualityMatch = note.match(/(\d+p?)/i);
        if (qualityMatch) return qualityMatch[1]?.toLowerCase().endsWith('p') ? qualityMatch[1].toLowerCase() : `${qualityMatch[1]}p`;
    }
    return undefined;
};
    ...
    try {
        ...
        try {
            ...
            const options: FormatOption[] = videoFormats.map((f) => {
                ...
                const bitrate = (f as any).bitrate ? Number((f as any).bitrate) : undefined;
            ...
            const uniqueExts = Array.from(new Set(options.map((o) => o.ext).filter(Boolean))).sort();
            const uniqueQualities = Array.from(new Set(options.map((o) => o.quality).filter(Boolean))).sort((a, b) => {
                const numA = parseInt(a?.replace('p', '') || '0');
                const numB = parseInt(b?.replace('p', '') || '0');
                return numB - numA; // Sort descending (highest first)
            });
            const uniqueBitrates = Array.from(new Set(options.map((o) => o.bitrate).filter((b): b is number => b !== undefined)))
            ...
            const options: FormatOption[] = videoOnlyOrProgressive.map((f: any) => {
                ...
                const bitrate = f.tbr ? Number(f.tbr) * 1000 : (f.vbr ? Number(f.vbr) * 1000 : undefined); // Convert kbps to bps
