...
export const runEarningsPrediction = (stats: any, sentiment?: any, comments?: any[]): Promise<any> => {
    return new Promise((resolve, reject) => {
        const pythonScriptPath = path.join(__dirname, '../ml/earnings_prediction.py');
        const pythonProcess = spawn('python', [pythonScriptPath], {
            stdio: ['pipe', 'pipe', 'pipe']
        });
        ...
        try {
            const inputData = JSON.stringify({ stats, sentiment, comments });
            pythonProcess.stdin.write(inputData);
            pythonProcess.stdin.end();
        } catch (err) {
            pythonProcess.kill();
